...
export class Network {
    ...
    retransmissionStrategy: "baseline" | "slot-based" | "SW2" | "SW3";
    ...
    addLink(nodeA: string, nodeB: string, etx: number): boolean {
        if (etx < 1) return false;
            ...
            if (!this.nodes.has(nodeB)) this.nodes.set(nodeB, new NetworkNode(nodeB));
    ...
    private addLinkBetween(a: string, b: string, etx: number): void {
        let nodeA = this.nodes.get(a);
        let nodeB = this.nodes.get(b);

        if (nodeA === undefined || nodeB === undefined) throw new Error("Can't add link between nonexistent nodes.")
     ...
     */
    getLinkEtx(idA: string, idB: string): number {
