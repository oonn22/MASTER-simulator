...
export class Scheduler {
     ...
     * @param network
     */
    static createSchedule(flows: Flow[], network: Network): Schedule {
    ...
    private static reverseLongestPathFirst(flows: Flow[], network: Network): Schedule {
        let schedule: SlotFrame[] = [];

        flows.sort((a, b): number => {
        ...
        for (let flow of flows) {
            ...
            for (let i = flow.hops.length - 1; i >= 0; i--) { //iterate over hops from last to first
                ...
                if (schedule[timeSlot] === undefined) schedule[timeSlot] = new SlotFrame(network.numChannels);
