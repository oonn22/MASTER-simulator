...
export class SlotFrame {
    readonly channels: Hop[]
    readonly numChannels: number;
    readonly scheduledNodes: Set<string>;

    constructor(numChannels: number) {
        this.numChannels = numChannels;
        this.channels = [];
        this.scheduledNodes = new Set<string>();
    }
     ...
     */
    scheduleTransmission(t: Hop): boolean {
        if (this.channels.length === this.numChannels) return false;

        for (let i = 0; i < t.sources.length; i++) {
            if (this.scheduledNodes.has(t.sources[i]) || this.scheduledNodes.has(t.destinations[i])) return false;
        }
