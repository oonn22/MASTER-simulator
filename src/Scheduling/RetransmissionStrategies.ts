 ...
 * @param network
 */
export function Baseline(flow: Flow, network: Network): void {
    flow.hops = []
 ...
 * @param network
 */
export function SlotBased(flow: Flow, network: Network): void {
    flow.hops = []
    ...
    for (let i = 0; i < flow.route.length - 1; i++) {
        ...
        if (linkEtx === 0) throw new Error("Can't transmit between unconnected nodes.");
 ...
 *                        of higher latency.
 */
export function SlidingWindow(flow: Flow, network: Network, equationNumber: 2 | 3, scalingFactor: number): void {
    flow.hops = [];
    ...
    for (let route of subFlows) {
        ...
        let windowStart = 0;
        let windowEnd = 1;
        let timesTransmitted: { [key: number]: number } = {};
        ...
        for (let _ = 0; _ < numTransmissions; _++) {
            ...
            //adjust window size
            if (timesTransmitted[windowStart] === windowSize) windowStart++;
            if (windowEnd - windowStart < windowSize && windowEnd !== route.length - 1) windowEnd++;
 ...
 */
function determineSubFlows(route: string[], maxSubFlowLength: number): string[][] {
    let routes = [];
    ...
    if (route.length > maxSubFlowLength) {
        ...
        for (let i = 0; i < numSubFlows; i++) {
            if (i == 0) routes.push(route.slice(0, subFlowLength));
            else routes.push(route.slice((i * subFlowLength) - 1, (i + 1) * subFlowLength));
        }
 ...
 *                                                            active in.
 */
function determineFlowCosts(route: string[], network: Network, equationNumber: 2 | 3, scalingFactor: number): [numTransmissions: number, windowSize: number] {
    let cost = 0;

    for (let i = 0; i < route.length - 1; i++) {
        if (equationNumber === 2) cost += network.getLinkEtx(route[i], route[i + 1]);
        else cost += Math.ceil(network.getLinkEtx(route[i], route[i + 1]))
    }

    if (equationNumber === 2) cost = Math.ceil(cost);
