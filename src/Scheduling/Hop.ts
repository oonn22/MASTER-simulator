...
export class Hop {
    ...
    constructor(source: string | string[], destination: string | string[], flowId: number) {
        this.flowId = flowId;

        if (typeof source === "string") this.sources = [source];
        else this.sources = source;

        if (typeof destination === "string") this.destinations = [destination];
        else this.destinations = destination;
    }
